...
public:
	...
	{
		Clear();
		SetReq(resourceName, method);
		SetHead("Connection", "close");
		SetHead("Host", hostName);
...
public:
	...
	int recv(HttpMsg* msg, size_t max = 1024, string resourceName = "/", const char root[] = "recive\\", float waitSec = 3)
	{
		...
		msg->split(_msg);
		if (msg->GetSta() != 200)return 0;

		//接收数据
		if (msg->GetHead("Content-Length") != "")
		{
			size_t size = atoi(msg->GetHead("Content-Length").c_str());
			//multipart/form-data
			if (msg->GetHead("Content-Type") == "multipart/form-data")
			{
				//分隔符
				string bdy;
...
public:
	...
	string GetHtml(const string resourceName = "/", float waitSec = 3)
	{
		...
		smsg.SetReq(a + b + c);
		smsg.SetHead("Connection", "keep-alive");
		smsg.SetHead("Host", host);
		//用Edge的UA
		smsg.SetHead("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36 Edg/109.0.1518.70");
		s.Send(smsg, waitSec);
